...
uint32_t width2bits(float width){
    // float fac = 8.192; //1khz
    // float fac = 8.192; //1khz
    // float fac = 8.192; //1khz
    float fac = 409.6; //50hz range 0.544-2.4ms (0-180deg)
    float conversion = width*fac; 
    uint32_t conv = conversion;
    return conv;
...
void app_main(void)
{
    ...
    ledc_channel_config(&ledc_chan_conf);
    ledc_set_duty(ledc_speed, pwm_chan, width2bits(2.4));
